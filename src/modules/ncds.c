...
#include "wav.h"
	...
	DUH * ncds_duh_backup = NULL;
	AL_DUH_PLAYER *ncds_dplayer_backup = NULL;
#else
	void * ncds_duh = NULL;
...
int ncds_loop =              0;
int ncds_done =              1;
int ncds_fine_tune =         0;
int ncds_mix_frequency = 44100;
...
char * ncds_get_music(char * fn)
{
	...
	if(ncds_music_on && ncds_initialized)
	{
		...
		{
			if(exists(replace_extension(ncds_filename, fn, ncds_music_type[i], 1024)))
			{
				return ncds_filename;
			}
		}
	}
	return NULL;
...
void ncds_set_fine_tune(int tune)
{
	ncds_fine_tune = tune;
}

void ncds_set_mix_frequency(int freq)
{
	ncds_mix_frequency = freq;
}
...
void ncds_play_mp3(char * fn, int loop)
{
		...
		if(ncds_music_on && ncds_initialized)
		{
			...
			ncds_mp3 = almp3_create_mp3(ncds_data, file_size_ex(fn));
        	almp3_play_ex_mp3(ncds_mp3, 8192, (int)((float)ncds_music_volume * 2.5), 128, 1000 + ncds_fine_tune, loop);
...
void ncds_play_ogg(char * fn, int loop)
{
		...
		if(ncds_music_on && ncds_initialized)
		{
			ncds_stop_music();
			ncds_data = buffer_file(fn);
			ncds_ogg = alogg_create_ogg_from_buffer(ncds_data, file_size_ex(fn));
        	alogg_play_ex_ogg(ncds_ogg, 8192, (int)((float)ncds_music_volume * 2.5), 128, 1000 + ncds_fine_tune, loop);
			ncds_set_music_volume(ncds_music_volume);
			ncds_loop = loop;
			ncds_done = 0;
		}
...
void ncds_play_s3m(char * fn, int loop)
{
		...
		if(ncds_music_on && ncds_initialized)
		{
			...
			ncds_dplayer = al_start_duh(ncds_duh, 2, 0, 1.0, NCDS_BUFFER_SIZE, ncds_mix_frequency);
			ncds_duh_backup = ncds_duh;
			ncds_dplayer_backup = ncds_dplayer;
			ncds_set_music_volume(ncds_music_volume);
			ncds_loop = loop;
			ncds_done = 0;
			if(!loop)
		...
		if(ncds_sample_queue[0])
		{
			...
			if(ncds_queue_channel < 0)
			{
				ncds_queue_channel = allocate_voice(ncds_sample_queue[0]);
       			...
       			voice_set_frequency(ncds_queue_channel, ncds_sample_queue[0]->freq + (ncds_sample_queue[0]->freq * ncds_fine_tune) / 1000);
...
void ncds_update_music(void)
{
	...
	if(ncds_initialized && !ncds_done)
	{
		...
		{
			#ifdef NCDS_DUMB
				if(ncds_duh_backup != ncds_duh || ncds_dplayer_backup != ncds_dplayer)
				{
					allegro_message("music pointer screwed!");
				}
				else
				{
...
void ncds_resume_music(void)
{
	...
	{
		#ifdef NCDS_ALMP3
        	almp3_play_ex_mp3(ncds_mp3, 8192, (int)((float)ncds_music_volume * 2.5), 128, 1000 + ncds_fine_tune, 1);
        #endif
	}
	else if(ncds_ogg)
	{
		#ifdef NCDS_ALOGG
        	alogg_play_ex_ogg(ncds_ogg, 8192, (int)((float)ncds_music_volume * 2.5), 128, 1000 + ncds_fine_tune, 1);
        #endif
	}
}
...
void ncds_set_position(unsigned long pos)
{
	if(ncds_dplayer)
	{
		...
		ncds_dplayer = al_duh_encapsulate_sigrenderer(ncds_sigrenderer, 1.0, NCDS_BUFFER_SIZE, ncds_mix_frequency);
		ncds_set_music_volume(ncds_music_volume);
		ncds_done = 0;
		if(!ncds_loop)
		{
...
int ncds_play_sample(SAMPLE * sp, int pan, int freq, int priority)
{
	int ret = -1;
	if(ncds_initialized && ncds_sound_on)
	{
		if(sp)
		{
			...
			if(freq < 0)
			{
				freq = 1000 + ncds_fine_tune;
			}
			ret = play_sample(sp, (int)(2.55 * (float)ncds_sound_volume), pan, freq, 0);
		}
	}
	return ret;
}
...
/* simplified sample playing function */
int ncds_play_sample_ex(SAMPLE * sp, int vol, int pan, int freq, int priority)
{
	int ret = -1;
	if(ncds_initialized && ncds_sound_on)
	{
		if(sp)
		{
			...
			ret = play_sample(sp, (int)(2.55 * (float)vol), pan, freq, 0);
		}
	}
	return ret;
}
